    ...
    return (
      <div className="max-w-7xl mx-auto px-4 py-8 animate-fadeIn">
        ...
        <button
          ...
          className="inline-flex items-center gap-2 text-edu-primary font-hanken text-xs font-bold uppercase mb-8 hover:text-edu-secondary transition-colors group cursor-pointer"
        >
        ...
        <header className="mb-10 max-w-4xl mx-auto text-center">
          <div className="inline-block bg-edu-secondary/15 text-edu-primary px-4 py-1.5 rounded-full font-hanken text-xs font-bold uppercase tracking-wider mb-4">
            {article.category}
          </div>
          <h1 className="font-serif text-3xl md:text-5xl font-bold text-edu-dark leading-tight mb-6">
            {article.title}
          </h1>
          <div className="flex flex-wrap items-center justify-center gap-4 md:gap-6 text-slate-500 font-hanken text-xs">
        ...
        <article className="max-w-3xl mx-auto font-hanken text-slate-700 text-base md:text-lg leading-relaxed space-y-6">
          ...
          {article.content.map((paragraph, index) => (
            <React.Fragment key={index}>
              ...
              {index === 0 && article.blockquote && (
                <blockquote className="border-l-4 border-edu-secondary bg-slate-50 p-6 md:p-8 my-8 italic font-serif text-lg md:text-xl text-edu-primary rounded-r-xl">
                  "{article.blockquote}"
                </blockquote>
              )}
        ...
        <section className="max-w-5xl mx-auto">
          <div className="flex items-center justify-between mb-8">
            <h3 className="font-serif text-2xl font-bold text-edu-dark">Noticias Relacionadas</h3>
            <button
              onClick={handleBackToList}
              className="text-edu-secondary font-hanken text-xs font-bold uppercase hover:text-edu-primary transition-all hover:underline"
            >
              Ver todas
            </button>
          </div>
                ...
                >
                  <div className="relative h-40 overflow-hidden">
                    ...
                    <div className="absolute top-3 left-3">
                      <span className="px-2.5 py-0.5 bg-white/90 text-edu-primary font-hanken text-[9px] font-bold uppercase rounded-full shadow-sm">
                        {related.category}
                      </span>
                    </div>
                  </div>
                  <div className="p-5">
                    <h4 className="font-serif text-sm font-bold text-edu-dark mb-2 line-clamp-2 leading-snug group-hover:text-edu-secondary transition-colors duration-300">
                      {related.title}
                    </h4>
                    <p className="font-hanken text-xs text-slate-500 line-clamp-2 leading-relaxed">
                      {related.lead}
                    </p>
                  </div>
                </div>
    ...
    <div className="animate-fadeIn">
      ...
      >
        ...
        <div className="relative z-10 text-center px-4 max-w-4xl mx-auto w-full flex flex-col items-center gap-6">
          <div className="space-y-2">
            <h1 className="text-4xl md:text-6xl font-serif font-bold text-white tracking-tight">Noticias y Novedades</h1>
            <p className="font-hanken text-lg md:text-xl text-slate-200 max-w-2xl mx-auto leading-relaxed">
          ...
          <div className="w-full max-w-2xl relative mt-4">
            <input
              ...
              className="w-full bg-white py-4 pl-6 pr-12 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-edu-secondary focus:border-edu-secondary font-hanken text-sm text-slate-800 shadow-lg transition-all"
            />
        ...
        <div className="max-w-7xl mx-auto px-4 flex flex-wrap gap-2 justify-center">
          {categories.map((category) => (
            <button
              ...
              className={`px-5 py-2 rounded-full font-hanken text-xs font-bold tracking-wider uppercase transition-all cursor-pointer ${
      ...
      <section className="py-10">
        {filteredArticles.length === 0 ? (
          <div className="text-center py-20 text-slate-400 font-hanken text-sm">
            ...
            {featuredNews && (
              <div className="max-w-7xl mx-auto px-4 py-6">
                <div className="mb-6 border-b border-slate-200 pb-3">
                  <h2 className="font-serif text-2xl font-semibold text-edu-dark">Destacado</h2>
                </div>
                <div
                ...
                >
                  <div className="w-full md:w-2/3 h-[280px] md:h-full relative overflow-hidden">
                    ...
                    <div className="absolute bottom-6 left-6 md:hidden">
                      <span className="inline-block px-3 py-1 bg-edu-secondary/20 text-white font-hanken text-xs font-bold uppercase tracking-wider rounded-full backdrop-blur-md border border-white/20 mb-3">
                  ...
                  <div className="w-full md:w-1/3 bg-white p-8 flex flex-col justify-center relative z-10 md:-ml-16 md:my-8 md:rounded-2xl md:shadow-2xl md:border border-slate-100 transition-transform duration-500 group-hover:-translate-y-1">
                    <div className="hidden md:block mb-3">
                      <span className="inline-block px-3 py-1 bg-edu-primary/10 text-edu-primary font-hanken text-xs font-bold uppercase tracking-wider rounded-full">
                        {featuredNews.category}
                      </span>
                    </div>
                    <div className="flex items-center gap-2 mb-3 text-slate-400 font-hanken text-xs">
                      <Calendar size={14} />
                      <span>{featuredNews.date}</span>
                    </div>
                    <h3 className="hidden md:block font-serif text-2xl font-bold text-edu-dark mb-4 leading-tight group-hover:text-edu-secondary transition-colors duration-300">
                      {featuredNews.title}
                    </h3>
                    <p className="font-hanken text-sm text-slate-500 mb-6 line-clamp-3 leading-relaxed">
                      {featuredNews.lead}
                    </p>
                    <button className="inline-flex items-center gap-2 font-hanken text-xs font-bold uppercase text-white bg-edu-primary px-6 py-3.5 rounded-xl hover:bg-edu-secondary transition-colors self-start shadow-md">
            ...
            {regularNews.length > 0 && (
              <div className="max-w-7xl mx-auto px-4">
                <div className="flex items-center justify-between mb-8 border-b border-slate-200 pb-3">
                  <h2 className="font-serif text-2xl font-semibold text-edu-dark">Últimas Noticias</h2>
                </div>
                    ...
                    >
                      <div className="relative h-[220px] overflow-hidden">
                        ...
                        <div className="absolute top-4 left-4">
                          <span className="px-3 py-1 bg-white/90 text-edu-primary font-hanken text-[10px] font-bold uppercase tracking-wider rounded-full shadow-sm backdrop-blur-sm">
                            {article.category}
                          </span>
                        </div>
                      </div>
                      <div className="p-6 flex flex-col flex-grow">
                        ...
                        <h3 className="font-serif text-lg font-bold text-edu-dark mb-2 line-clamp-2 leading-tight group-hover:text-edu-secondary transition-colors duration-300">
                          {article.title}
                        </h3>
                        <p className="font-hanken text-xs text-slate-500 mb-4 line-clamp-2 flex-grow leading-relaxed">
                          {article.lead}
                        </p>
                        <div className="inline-flex items-center gap-1 font-hanken text-[11px] font-bold uppercase text-edu-primary group-hover:text-edu-secondary transition-colors mt-2">
                          <span>Leer más</span>
            ...
            <div className="flex gap-2 justify-center pt-8">
              {[1, 2, 3].map((page) => (
                <button
                  ...
                  className={`w-9 h-9 rounded-lg font-hanken text-xs font-bold transition-all flex items-center justify-center cursor-pointer ${
                    activePage === page
                      ? 'bg-edu-primary text-white shadow-md'
              ...
              <button
                ...
                className="w-9 h-9 rounded-lg font-hanken text-xs font-bold bg-slate-100 text-slate-500 hover:bg-slate-200 flex items-center justify-center cursor-pointer"
              >
                ›
              </button>
            </div>
